...
my $usage = "\nUsage [$version]: 
    perl $scriptname  -d <directory> -g <directory> -t <directory> [-p <path of the outputdirectory>][-o <output file>] [-v] [-c] [-h] 
...
die "\n -d $gdir does not exist?\n\n"  if (! -e $gdir);
die "\n -d $TEdir does not exist?\n\n"  if (! -e $TEdir);
$rdir = $1 if ($rdir =~ /^(.*)\/$/);
$gdir = $1 if ($gdir =~ /^(.*)\/$/);
$TEdir = $1 if ($TEdir =~ /^(.*)\/$/);
...
#rename TEfiles
# my @TEfiles = `ls $TEdir`;
# foreach my $TE (@TEfiles){
# 	chomp $TE;
# 	my @filename = split (/\./,$TE);
# 	$TEinfo{$filename[0]} = $TE; 
# }
#print Dumper %TEinfo;
...
# 	}
	...
	print STDERR "the query file is $queryte\n";
	&blast_file ($TEdir,"$queryte.fasta",$directory);	#blast with the query TEsequence
	my $blastfile = "$outpath/$queryfile.tabular.blast.out";
...
&print_keyvalue($fineout,\%strandprediction);
...
foreach my $locus (sort keys %extractcontiginfo) {
	...
	if (defined ($tsdinfo)) {
		($testart,$teend) = &findcord_bothtsds($tsdinfo);
	...
	push (@teseq_array,$seqout_obj);
}
&printarray_fasta ($teout,@teseq_array);
exit;
#-----------------------------------------------------------------------------
#----------------------------------- SUB -------------------------------------
#-----------------------------------------------------------------------------
