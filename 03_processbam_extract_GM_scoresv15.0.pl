...
my $usage = "\nUsage [$version]: 
    perl $scriptname -t <table> -f <split files> -g <pathtogenomefile> -bl <bam location> -m <mappability give yes or no> [-p <path of the outputdirectory processbamout>][-db<mysql dbinfo>] [-u<user>] [-pd <password>]  [-v] [-c] [-h] [-s] 
    ...
    -m,--map.score    	(STRING) yes/no if you want mappability score (construction of a mysql database would be neccessary) 
...
#------------------------------ LOAD AND CHECK -------------------------------
#-----------------------------------------------------------------------------
my ($file,$path,$table,$GENOME,$bamlocation,$picardtools,$mappability,$mysqltable,$mysqldb,$user,$password,$verbose,$help,$v,$chlog);
GetOptions ('f=s' => \$file,
            'p=s' => \$path,
            'g=s' => \$GENOME,
            't=s' => \$table,
...
&print_AOH (%allindimappingscores) if ($mappability eq "yes");
# Disconnect from the database.
$dbh->disconnect() if ($mappability eq "yes");
...
sub extractgmscores {
	...
	my @mappingscores = ();
	my $mappingscore;
	#foreach my $location (keys %cordinates) {
		...
		my $start5 = $gl[2];
		my $end5 = $gl[3];
		my $givencordinate = $chro.":".$start5."-".$end5;#250 flanks
		if (exists ($cordinatescore{$givencordinate})) {
			my $value = $cordinatescore{$givencordinate};
			#$mappingscore = $value;
		...
		#my $chro = $gl[1];
		$start5 = $gl[2] - 250;#500 cordinates
		$end5 = $gl[3] + 250;
		$givencordinate = $chro.":".$start5."-".$end5;
		if (exists ($cordinatescore{$givencordinate})) {
			my $value = $cordinatescore{$givencordinate};
			#$mappingscore = $value;
			push (@mappingscores,$value);
		...
		#push (@{$allindimappingscores{$location}},$mappingscore);
		#}
	#}
	$allindimappingscores{$location} = [@mappingscores];
...
sub extractgmscores_mysql {
	...
	if ($count >= 1) {
		...
		$cordinatescore{$gposition} = $len_avg;
	}
	return ($len_avg);	
...
sub print_AOH {
	...
	print $mp "individual_and_Locus\tMappability_score_500bp_flank\tMappability_score_250bp_flank\n";
