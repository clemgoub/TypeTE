...
open (my $fh, "<", $file) or confess "\n ERROR (main): could not open to read $file $!\n";
	while(<$fh>) {
		...
		system ("samtools view -b -f 2048 $bamlocation/$bamid $genomeloc > $path/splitreads/$genomeloc/$uniqueid.splitF2048.outfile.bam") == 0 or die ("unable to extract readsby F 3854 flag $!");
