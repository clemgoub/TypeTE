...
#
#	- v2.0 = 12 April 2018
#				going to do a weighted average for the scores instead of simply average 
#
#
#
#		
...
print STDERR "Fetching data from mysql database..\n";
	while(<$fh>) {
		...
		&extractgmscores($uniqueid,$genomeloc,$strand);
	}
close $fh;
...
sub extractgmscores {
	...
	my $rightscore = &store_mapscores($givencordinate);#output only score here score,weighted score
	print "right score $length is $rightscore\n" if ($verbose);
	if ($strandin eq '+') {
		@fivepscore = split /,/,$leftscore;
		@threepscore = split /,/, $rightscore;
	} elsif (($strandin eq 'C') || ($strandin eq '-')) {
		@fivepscore = split /,/, $rightscore;
		@threepscore = split /,/,$leftscore;
	}
	
	#my $avgscore = ($leftscore+$rightscore)/2;#
	my $avgscore = ($fivepscore[0]+$threepscore[0])/2;#
	my $avgwscore = ($fivepscore[1]+$threepscore[1])/2;#
	my $alltabs = $avgscore."\t".$avgwscore."\t".$fivepscore[0]."\t".$threepscore[0]."\t".$fivepscore[1]."\t".$threepscore[1];
	my $gmscore = $length."=>".$alltabs;
	push (@mappingscores,$gmscore);
	$length = 500;
	...
	#my $avgscore = ($leftscore+$rightscore)/2;#
	$avgscore = ($fivepscore[0]+$threepscore[0])/2;#
	$avgwscore = ($fivepscore[1]+$threepscore[1])/2;#
	$alltabs = $avgscore."\t".$avgwscore."\t".$fivepscore[0]."\t".$threepscore[0]."\t".$fivepscore[1]."\t".$threepscore[1];
	$gmscore = $length."=>".$alltabs;
	push (@mappingscores,$gmscore);
...
sub print_AOH {
	...
	print $mp "Locus\tavgscore500bp\tavgwscore500bp\tfivepscore500bp\tthreepscore500bp\tfivepweiscore500bp\tthreepweiscore500bp\tavgscore250bp\tavgwscore250bp\tfivepscore250bp\tthreepscore250bp\tfivepweiscore250bp\tthreepweiscore250bp\n";
