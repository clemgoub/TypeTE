...
then
    perl 01_DelP_findcorrespondinginsertion_v3.2.pl -t $RM_TRACK -f $OUTDIR/$PROJECT/$PROJECT.input -p $OUTDIR/$PROJECT/RM_intervals.out
else
    perl 01_DelP_findcorrespondinginsertion_v3.2.pl -t <(sed 's/chr//g' $RM_TRACK) -f $OUTDIR/$PROJECT/$PROJECT.input -p $OUTDIR/$PROJECT/RM_intervals.out
fi
